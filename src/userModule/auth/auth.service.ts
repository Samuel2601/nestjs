...
export class AuthService {
  ...
  async outlookLogin(code: string) {
    ...
    const { data } = await this.httpService.get('https://graph.microsoft.com/v1.0/me', {
  ...
  private async handleSocialLogin(provider: string, providerId: string, userData: any) {
    let user = await this.userModel.findOne({ 'redes.providerId': providerId, 'redes.provider': provider });
